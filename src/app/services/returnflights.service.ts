...
export class ReturnflightsService {
  ...
  public getreturnFlightsFromApi(travel_date:string,source_destination:string,target_destination:string,travellers:number)
    {
        return this.returnflights.get("http://localhost:62438/api/Returnflights?travel_date="+travel_date+"&source_destination="+source_destination+"&target_destination="+target_destination+"&no_traveller="+travellers);
